...
class OAuthListener extends AbstractAuthenticationListener
{
    ...
    private function transformOAuthError($errorCode)
    {
        ...
        switch ($errorCode) {
            case 'access_denied':
                return $trans->trans('social.error.access_denied', array(), 'UserBundle');

            case 'authorization_expired':
                return $trans->trans('social.error.authorization_expired', array(), 'UserBundle');

            case 'bad_verification_code':
                return $trans->trans('social.error.bad_verification_code', array(), 'UserBundle');

            case 'consumer_key_rejected':
                return $trans->trans('social.error.consumer_key_rejected', array(), 'UserBundle');

            case 'incorrect_client_credentials':
                return $trans->trans('social.error.incorrect_client_credentials', array(), 'UserBundle');

            case 'invalid_assertion':
                return $trans->trans('social.error.invalid_assertion', array(), 'UserBundle');

            case 'redirect_uri_mismatch':
                return $trans->trans('social.error.redirect_uri_mismatch', array(), 'UserBundle');

            case 'unauthorized_client':
                return $trans->trans('social.error.unauthorized_client', array(), 'UserBundle');

            case 'unknown_format':
                return $trans->trans('social.error.unknown_format', array(), 'UserBundle');
        }

        return sprintf($trans->trans('social.error.unknown_error', array('%errorCode%' => $errorCode ), 'UserBundle'));
    }
}
